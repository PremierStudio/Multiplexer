...
2. **Native, blazing-fast editor** (Rust + GPUI, GPU-rendered) as a **review pane or pop-out**, never the Outlook center. Inline diff-apply, LSP, multi-cursor, Vim mode. Grok edits. Multiplexer reviews.
...
- **Embedded harness:** fork/vendor `xai-org/grok-build` (Apache 2.0) under `third_party/` (or `[patch]`). Reuse `xai-grok-shell` (agent runtime), `xai-grok-tools`, `xai-grok-workspace` as libraries. **Host** `xai-grok-pager` (the real Grok TUI). Do not rebuild the pager in GPUI. Chat log is `grok -p` until in-process Engine C. **Windows build support is our responsibility** (upstream says Windows is best-effort/untested). **In-process embedding is a Phase-0 go/no-go hypothesis (D10)** — the ACP path is the fallback. The running desktop is Outlook chrome + `grok -p` + a TUI host, not the full differentiator list yet.
