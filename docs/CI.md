...
The CI invocation uses `--in-place` to avoid the default copy-tree mode (which copies the whole tree, roughly 30 GB here) while still writing the same `mutants.out/` format. The version is pinned to **27.1.0**, the version that produced the existing `mutants.out/` report.
...
cargo mutants --in-place --timeout 30
