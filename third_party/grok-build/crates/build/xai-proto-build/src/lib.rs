...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        for proto in protos {
            ...
            if cfg!(windows) {
                // Windows has no /dev/stdout or /dev/null device paths; protoc
                // rejects them with "No such file or directory", and a bare
                // "NUL" is treated as a literal filename in some contexts.
                // Use temp files for both outputs, then read them back.
                // (Multiplexer fork patch.)
                let base = format!(
                    "xai-proto-build-{}",
                    std::process::id()
                );
                let dep_out = std::env::temp_dir().join(format!(
                    "{base}-{}.d",
                    proto.file_name().unwrap_or_default().to_string_lossy()
                ));
                let desc_out = std::env::temp_dir().join(format!(
                    "{base}-{}.desc",
                    proto.file_name().unwrap_or_default().to_string_lossy()
                ));
                let _ = std::fs::remove_file(&dep_out);
                let _ = std::fs::remove_file(&desc_out);
                command
                ...
                let output = std::fs::read_to_string(&dep_out)
                    .context("read protoc dependency output")?;
                let _ = std::fs::remove_file(&dep_out);
                let _ = std::fs::remove_file(&desc_out);
                let mut lines = output.lines();
                let first_line = lines.next().context("protoc command output is empty")?;
                // On Windows protoc emits the dependency target as
                // "<desc_out>: <path>"; strip the whitespace-separated target
                // label (anything up to the first ": ") to get the first dep.
                let rem = first_line
                    .split_once(": ")
                    .map(|(_, rest)| rest)
                ...
                let output = String::from_utf8(output.stdout).context("protoc command output not UTF-8")?;
